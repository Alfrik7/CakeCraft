...
describe('price helpers', () => {
  it('estimateWeightKg uses guest-to-weight formula', () => {
    ...
    expect(estimateWeightKg(4)).toBe(0.5);
    expect(estimateWeightKg(8)).toBe(1);
    expect(estimateWeightKg(10)).toBe(1.25);
    expect(estimateWeightKg(20)).toBe(2.5);
  });

  it('getItemPrice handles fixed and per_kg items', () => {
    expect(getItemPrice(10, { price: 500, price_type: 'fixed' })).toBe(500);
    expect(getItemPrice(10, { price: 700, price_type: 'per_kg' })).toBeCloseTo(875);
  });

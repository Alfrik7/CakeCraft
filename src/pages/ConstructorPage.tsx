...
import { MenuDataProvider, useMenuDataContext } from '../context/MenuDataContext';
...
function ConstructorPageContent({ baker }: { baker: Baker }) {
  const { isMenuLoading } = useMenuDataContext();

  if (isMenuLoading) {
    return <ConstructorPageSkeleton />;
  }

  return <ConstructorLayout baker={baker} />;
}
...
export function ConstructorPage() {
  ...
  return (
    <OrderProvider bakerId={baker.id}>
      <MenuDataProvider bakerId={baker.id}>
        <ConstructorPageContent baker={baker} />
      </MenuDataProvider>
    </OrderProvider>
  );
}
